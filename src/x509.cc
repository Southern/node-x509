...
static const char *MISSING[4][2] = {
    ...
    {"1.3.6.1.4.1.311.60.2.1.1", "jurisdictionOfIncorpationLocalityName"},
...
NAN_METHOD(verify) {
    ...
    std::string cert_path = *Nan::Utf8String(info[0]->ToString());
    std::string ca_bundlestr = *Nan::Utf8String(info[1]->ToString());
...
NAN_METHOD(get_altnames) {
    ...
    info.GetReturnValue().Set(Nan::Get(exports, key).ToLocalChecked());
    ERR_clear_error();
}
...
Local<Value> try_parse(const std::string &dataString) {
    ...
    Nan::Set(exports, Nan::New<String>("subject").ToLocalChecked(),
             parse_name(X509_get_subject_name(cert)));
    Nan::Set(exports, Nan::New<String>("issuer").ToLocalChecked(),
             parse_name(X509_get_issuer_name(cert)));
    Nan::Set(exports, Nan::New<String>("serial").ToLocalChecked(),
             parse_serial(X509_get_serialNumber(cert)));
    Nan::Set(exports, Nan::New<String>("notBefore").ToLocalChecked(),
             parse_date(X509_get_notBefore(cert)));
    Nan::Set(exports, Nan::New<String>("notAfter").ToLocalChecked(),
             parse_date(X509_get_notAfter(cert)));
    // Subject hash
    std::stringstream stream;
    stream << std::hex << X509_subject_name_hash(cert);
    Nan::Set(exports, Nan::New<String>("subjectHash").ToLocalChecked(),
    ...
    Nan::Set(exports, Nan::New<String>("signatureAlgorithm").ToLocalChecked(),
    ...
    if (X509_digest(cert, EVP_sha1(), md, &md_size)) {
        ...
        Nan::Set(exports, Nan::New<String>("fingerPrint").ToLocalChecked(),
    ...
    Nan::Set(publicKey, Nan::New<String>("algorithm").ToLocalChecked(),
    ...
    if (pkey_nid == NID_rsaEncryption) {
        ...
        Nan::Set(publicKey, Nan::New<String>("e").ToLocalChecked(),
                 Nan::New<String>(rsa_e_dec).ToLocalChecked());
        OPENSSL_free(rsa_e_dec);
        Nan::Set(publicKey, Nan::New<String>("n").ToLocalChecked(),
                 Nan::New<String>(rsa_n_hex).ToLocalChecked());
        OPENSSL_free(rsa_n_hex);
        Nan::Set(publicKey, Nan::New<String>("bitSize").ToLocalChecked(),
                 Nan::New<Uint32>(rsa_key_length_int));
    }
    Nan::Set(exports, Nan::New<String>("publicKey").ToLocalChecked(),
             publicKey);
    EVP_PKEY_free(pkey);
    ...
    names = (STACK_OF(GENERAL_NAME) *)X509_get_ext_d2i(
    ...
    if (names != NULL) {
        ...
        for (i = 0; i < length; i++) {
            ...
            if (current->type == GEN_DNS) {
                ...
                if (ASN1_STRING_length(current->d.dNSName) !=
                    (int)strlen(name)) {
                    ERR_clear_error();
    ...
    for (index_of_exts = 0; index_of_exts < num_of_exts; index_of_exts++) {
        ...
        // IFNULL_FAIL(ext_bio, "unable to allocate memory for extension value
        // BIO");
        ...
        if (nid == NID_undef) {
            ...
            Nan::Set(extensions,
                     Nan::New<String>(real_name((char *)c_ext_name))
                         .ToLocalChecked(),
                     Nan::New<String>(trimmed_data).ToLocalChecked());
        }
    ...
    Nan::Set(exports, Nan::New<String>("extensions").ToLocalChecked(),
             extensions);
...
Local<Value> parse_date(ASN1_TIME *date) {
    ...
    Local<Object> DateObject =
        Nan::Get(global, Nan::New<String>("Date").ToLocalChecked())
            .ToLocalChecked()
            ->ToObject();
    return scope.Escape(
        Nan::CallAsConstructor(DateObject, 1, args).ToLocalChecked());
}
...
Local<Object> parse_name(X509_NAME *subject) {
    ...
    for (i = 0; i < length; i++) {
        ...
        Nan::Set(cert, Nan::New<String>(real_name(buf)).ToLocalChecked(),
...
char *real_name(char *data) {
    ...
    for (i = 0; i < length; i++) {
        if (strcmp(data, MISSING[i][0]) == 0) return (char *)MISSING[i][1];
    }
